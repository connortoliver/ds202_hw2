...
Assuming this is not inclusive (i.e. 2000 is the highest number), we can assume that 2000 is the highest square. As such, if we take the square root of 2000 and round it down with `floor()`, we will get the closest perfect square root to sqrt(2000). This will be the number of square roots between 1 and 2001 not inclusive, since every square below the sqrt(2000) will be in the range.
...
By sorting the dataframe by `hwy` descending and then selecting the first 3 rows in the `mpg` dataset, we find that Volkswagen has the 3 most fuel efficient models in the dataset. The models are a 1999 4-cylinder Jetta with a manual transmission, a 1999 4-cylinder New Beetle with a manual transmission, and a 1999 4-cylinder New Beetle with an automatic transmission.
...
We can select a subset of the dataframe to select cars with the `class` equal to `compact`. Then we can use the `nrow()` function to select the number of rows in the dataframe to find the number of cars. In this dataframe, there are 47 compact cars.
...
According to the scatterplot, there is a positive correlation between `hwy` and `cty`, or highway mileage and city mileage. In other words, the more mileage a car gets on the highway, the more mileage it gets in the city. 

There are so few points visible because many points cover up the others, so they are hard to see.
...
According to the summary statistics, cars from 1999 appear to be slightly more fuel efficient overall compared to cars from 2008. Cars from 2008 have lower maximum and minimum city and highway mileage rates. The mean highway mileage is higher in 2008 by a mere 0.02, but 1999 still is more efficient overall. Let's look at some box plots for some visualizations.

```{r}
par(mfrow=c(1,4))
boxplot(mpg2008$hwy, main="2008 Highway")
boxplot(mpg2008$cty, main="2008 City")
boxplot(mpg1999$hwy, main="1999 Highway")
boxplot(mpg1999$cty, main="1999 City")
```

We can see that there are some outliers (probably the 1999 Volkswagens from earlier) might skew the data. The 3rd quartile for 2008 highway mileage is higheer than that of 1999 highway mileage. Without the outliers, the 2008 cars might be more fuel efficient. However, right now, it appears that the 1999 cars are more efficient. 